...
all_tpr = []
f1 = cross_validation.cross_val_score(clf, x, y , cv=10, scoring='f1').mean()
acc = cross_validation.cross_val_score(clf, x,y, cv=10, scoring='accuracy').mean()
auc = cross_validation.cross_val_score(clf, x, y , cv=10, scoring='roc_auc').mean()
ppv = cross_validation.cross_val_score(clf, x,y, cv=10, scoring='precision').mean()
tpr_per_fold = []
fpr_per_fold = []
npv_per_fold = []
auc_per_fold = []
...
title_str = 'Receiver operating characteristic: ' + 'Random Forest' + ', ALL Med'
title(title_str )
...
## plot AUC boxplot
data = [d_l_auc_all_featurespaces['HTNMED'], d_l_auc_all_featurespaces['HTN_FREQ'],d_l_auc_all_featurespaces['HTNMED_HTNFREQ'],d_l_auc_all_featurespaces['ALLMED']]
figure()
boxplot(data)
show()
