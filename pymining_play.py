...
for key in d_itemsets_per_pt:   
    l_freq_itemsets = d_itemsets_per_pt[key]
    l_itemsets_as_frozensets = [frozenset(x) for x in l_freq_itemsets]
    d_freq_itemsets_this_pt = dict( (fs, l_itemsets_as_frozensets.count(fs)) for fs in l_itemsets_as_frozensets)
    temp_copy = d_freq_itemsets_this_pt.copy()
    for fs_itemset in temp_copy: #loop thru dict of counts
