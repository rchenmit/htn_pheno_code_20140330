...
d_all_classifiers['LIN_SVC'] = dict()
...
for tr, ts in idx:
    ...
    model_linearsvc = mlpy.LibLinear(solver_type='l2r_l2loss_svc_dual') #default: mlpy.LibLinear(solver_type='l2r_lr', C=1, eps=0.01, weight={})
    model_linearsvc.learn(trainset_features, trainset_samples)
    #do the prediction on testing set
    y_predicted = np.array(model_linearsvc.pred(testset_features))
    #record results:
...
d_all_classifiers['LIN_SVC']['CLINICIANBP_MED'] = d_results
